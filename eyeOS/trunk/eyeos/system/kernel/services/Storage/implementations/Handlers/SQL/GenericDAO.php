...
class GenericDAO implements IStorageHandler, ISQLHandler, ITransactionHandler {
	...
	private static $ignoredMethods = array(
			'getMeta',
			'setMeta'
	);
	...
	public function getPrimaryKey($transferObject) {
		...
		for($i = 0;$i < $select->columnCount() - 1; $i++) {
	...
	private static function getTableName($transferObject, $prefix = null) {
		$pref = '';
		if ($prefix != null) {
			$pref = $prefix . '_';
		}
	...
	private static function getTransferObjectAttributes($transferObject) {
